...
public class Scene {
	...
	public Scene(int round) {
		map = new int[Consts.MAP_SIZE_HEIGHT][Consts.MAP_SIZE_WIDTH];
		List<GameMap> maps = MapManager.getInstance().getAllMaps();
		GameMap m = maps.get(round - 1);
		for (int i = 0; i < m.height; i++) {
			for (int j = 0; j < m.width; j++) {
				this.map[i][j] = m.data[i][j];
			}
		}
		...
		for (int i = 0; i < map.length; i++) {
			for (int j = 0; j < map[0].length; j++) {
				type = map[i][j];
				if (type == Consts.ELEMENT_TYPE_ROAD
						|| type == Consts.ELEMENT_TYPE_TANK) {
					continue;
				}
	...
	public synchronized void removeBoom(Boom b) {
